...
%
% Dirichlet boundary control  y=u 
%
% minimise 
...
U = forWaveBCD(F,CB,x,t,Nx,Nt);        %Calculate the corresponding state 
U_old=U;

P = adjWaveBCD(U,x,t,Nx,Nt,alpha,beta);   %Calculate the corresponding adjoint 

J(1) = JcostBCD(U,Y,CB,alpha,beta,nu,gamma,Nx,Nt);   %Calculate value cost functional 
...
for k=1:kmax
    ...
    for j=2:Nt  
        ...
        i=1;
            ...
            pp=P(j,i);
            uu=CB(j,1);
            dpp=-(P(j,i+1)-P(j,i))/dx;
            
            
            %Candidates where the augmented Hamiltonian takes its max 
            u1=min(max(F_lo,(2*epsilon*uu-v2*dpp+gamma)/(2*epsilon+nu)),0);  
            u2=min(max(0,(2*epsilon*uu-v2*dpp-gamma)/(2*epsilon+nu)),F_up);

            %Corresponding values of the augmented Hamiltonian
            H1=-v2*dpp*u1 - 0.5*nu*u1^2 - gamma*abs(u1) - epsilon*(u1-uu)^2;                     
            H2=-v2*dpp*u2 - 0.5*nu*u2^2 - gamma*abs(u2) - epsilon*(u2-uu)^2;
    ...
    U = forWaveBCD(F,CB,x,t,Nx,Nt);  %Calculation of the state equation with the updated control
  
    Jold = JcostBCD(U,Y,CB,alpha,beta,nu,gamma,Nx,Nt); %Corresponding cost functional value
    ...
    if(Jold-J(count_updates) > - eta * dF)  
        ...
        P = adjWaveBCD(U,x,t,Nx,Nt,alpha,beta);  
...
%
filename = sprintf('waveytkBCD%d.eps',k);
...
%  title('control left');
  print('-depsc2','waveuBCDleft.eps','-b0'); 
  
  figure(2+lpt);
  plot(tm,CB(:,2),'linewidth',2);
  xlabel('t','fontSize',14);
%  title('control right');
  print('-depsc2','waveuBCDright.eps','-b0'); 
...
xlabel('t');
view([135 30])
print('-depsc2', 'waveyBCD01.eps','-b0'); 
% print('-dpdf', 'waveyBCD01.pdf','-b0');

figure(15)
% plot(J,ltype{2},'Linewidth',2)
semilogx(E,ltype{3},'Linewidth',2);

print('-depsc2', 'waveEbcd01.eps','-b0'); 
print('-dpdf', 'waveEbcd01.pdf','-b0');
...
print('-depsc2', 'waveJbcd01.eps','-b0'); 
% print('-dpdf', 'waveJbcd01.pdf','-b0');
