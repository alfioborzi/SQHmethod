...
disp(' ');
%
% solving an elliptic linear optimal control problem on a
% square [0,a]x[0,b] - L2 and L1 costs
%
% A robust SQH scheme to solve non-smooth PDE optimal control problems
% by Tim Breitenbach and Alfio Borzi'
%
% Problem specific parameters: 
% (a,b) side length of the domain, 
...
OCP=struct('N',99,'a',0,'b',1,'u_lo',-100,'u_up',100,'alpha',10^-5,'beta',10^-3,'s',0); 
...
zeta=0.9;
sigma=1.1;
kmax=2000;        %Maximum iteration number
...
yd=sin(2*pi*X1).*cos(2*pi*X2)+1; %Desired state
...
%  title('The state variable');
  ...
  print('-depsc2', 'ellP11y.eps','-b0'); 
% print('-dpdf', 'ellP11y.pdf','-b0');
...
%  title('The control variable');
  ...
  print('-depsc2', 'ellP11u.eps','-b0'); 
% print('-dpdf', 'ellP11u.pdf','-b0');
...
%  title('Contour of the control variable');
  ...
  print('-depsc2', 'ellP11ucontour.eps','-b0'); 
% print('-dpdf', 'ellP11ucontour.pdf','-b0');
...
% title('Convergence history of J');
  ...
  print('-depsc2', 'ellP11Jhist.eps','-b0'); 
% print('-dpdf', 'ellP11Jhist.pdf','-b0');


fprintf('\n')
fprintf('Check numerical optimality of the SQH solution\n')
% Check solution for optimality; 
numeric_optimality(u,yd,OCP)
