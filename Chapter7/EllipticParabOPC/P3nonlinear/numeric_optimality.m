...
function [ ] = numeric_optimality(u,y,yd,A,OCP)
...
for i=1:n
    for j=1:n
        ...
        Hopt(i,j)=(OCP.alpha/2)*u(i,j).^2+OCP.beta*abs(u(i,j)).*(abs(u(i,j))>OCP.s)+u(i,j).*p(i+1,j+1);
        %Determine for y and p and the returned u again the value for the
        %control where the Hamiltonian takes its minimum
        unew(i,j)=argMinH(u(i,j),p(i+1,j+1),0,OCP);
    end
end
...
for i=1:n
    for j=1:n
        ...
        Hnew(i,j)=(OCP.alpha/2)*unew(i,j).^2+OCP.beta*abs(unew(i,j)).*(abs(unew(i,j))>OCP.s)+unew(i,j).*p(i+1,j+1); 
    end
end
