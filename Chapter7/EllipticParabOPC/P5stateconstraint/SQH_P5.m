...
disp(' ');
%
% solving an elliptic optimal control problem with state constraints 
% with fix penalty (gamma) in the cost functional
%
...
% c the upper bound for the state, gamma weight for the penalization term,
...
OCP=struct('N',99,'a',0,'b',1,'u_lo',-50,'u_up',50, ... 
    'alpha',10^-4,'beta',10^-3,'s',0,'c',0.3,'gamma',10000);
...
[X1,X2]=meshgrid(OCP.a:h:OCP.b,OCP.a:h:OCP.b);
[X1u,X2u]=meshgrid(OCP.a+h:h:OCP.b-h,OCP.a+h:h:OCP.b-h);
...
u_old=u;
y=forward_y(u,A,OCP);          %Calculate the corresponding state variable y
y_old=y;
...
for k=1:kmax
    ...
    for i=1:OCP.N    
       for j=1:OCP.N
            %Pointwise minimisation of the corresponding augmented
            %Hamiltonian with an exact formula
            pint=p(i+1,j+1);
            %Candidates for a minimum of the augmented Hamiltonian
            u1=min(max(OCP.s,(2*epsilon*u(i,j)-pint-OCP.beta)/(OCP.alpha+2*epsilon)),OCP.u_up); 
            u2=min(max(OCP.u_lo,(2*epsilon*u(i,j)-pint+OCP.beta)/(OCP.alpha+2*epsilon)),-OCP.s);
            u3=min(max(-OCP.s,(2*epsilon*u(i,j)-pint)/(OCP.alpha+2*epsilon)),OCP.s);            
            %Corresponding values of the augmented Hamiltonian
            H1=(OCP.alpha/2)*u1^2+OCP.beta*abs(u1)*(abs(u1)>OCP.s)+u1*pint+epsilon*((u1-u(i,j))^2);
            H2=(OCP.alpha/2)*u2^2+OCP.beta*abs(u2)*(abs(u2)>OCP.s)+u2*pint+epsilon*((u2-u(i,j))^2);
            H3=(OCP.alpha/2)*u3^2+u3*pint+epsilon*((u3-u(i,j))^2);
            %Choose the update for the control at the corresponding grid point that has the lowest 
            %value of the augmented Hamiltonian
            [~,pos]=min([H1,H2,H3]);        
            switch pos
                case 1
                    u(i,j)=u1;
    ...
    y=forward_y(u,A,OCP);           %Calculation of the state equation with the updated control u
...
%  title('The state variable');
  ...
  print('-depsc2', 'ellP5y.eps','-b0'); 
% print('-dpdf', 'ellP5y.pdf','-b0');
...
%  title('The control variable');
  ...
  print('-depsc2', 'ellP5u.eps','-b0'); 
% print('-dpdf', 'ellP5u.pdf','-b0');
...
%  title('Contour of the control variable');
  ...
  print('-depsc2', 'ellP5ucontour.eps','-b0'); 
% print('-dpdf', 'ellP5contour.pdf','-b0');
...
% title('Convergence history of J');
 ...
 print('-depsc2', 'ellP5Jhist.eps','-b0'); 
% print('-dpdf', 'ellP5Jhist.pdf','-b0');

fprintf('\n')
fprintf('Check numerical optimality of the SQH solution\n')
% Check solution for optimality; 
numeric_optimality(u,yd,A,OCP)

fprintf('Maximum value of y: %f\n', max(max(y)) )

cx=OCP.c;
y=y-cx; 
scount = h*h*length(nonzeros(y(y>0)));

fprintf('Measure of M: %f\n', scount)
